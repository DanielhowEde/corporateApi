...
class Message(BaseModel):
    ...
    def validate_test_id(cls, v: str) -> str:
        if not 3 <= len(v) <= 10:
            raise ValueError("TestID must be 3-10 characters")
        return v

    @field_validator("test_area")
    @classmethod
    def validate_test_area(cls, v: str) -> str:
        if not 3 <= len(v) <= 64:
            raise ValueError("Area must be 3-64 characters")
        return v
