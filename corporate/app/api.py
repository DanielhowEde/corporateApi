"""
Programmatic API (/api/v1) for external callers.

This surface is distinct from the web UI. Callers authenticate with a
client certificate issued by the corporate CA; the reverse proxy sets a
header containing the cert's Common Name. Each cert is bound to a
specific project at issue time; requests for any other project are
refused.

Two modes for sending a message:

- Template mode — POST /api/v1/messages/template/{project}/{name}
  Load {PAYLOADS_DIR}/{project}/{name}.json from disk, optionally merge
  caller-supplied overrides, regenerate ID + Date (unless verbatim=true),
  validate against the Message model plus every schema under
  {project}/Schemas/, then cert-wrap and forward via the gateway.

- Schema mode — POST /api/v1/messages/schema/{project}
  Caller supplies the full payload. Validated against every schema under
  {project}/Schemas/ (all must pass) and then sent.

GETs for listing templates + schemas are provided for discovery.
"""
...
# Module-level dependencies wired from lifespan. Tests can overwrite
# these directly (see conftest patterns used by user.py).
gateway_client = None
key_manager = None
...
def set_key_manager(manager) -> None:
    """Wire the key manager used for CN -> project lookup."""
    global key_manager
    key_manager = manager
...
def _extract_cn(request: Request) -> str | None:
    ...
    We accept two conventional header shapes:
      - X-Client-Cert-CN  (direct CN string)
      - X-Ssl-Client-S-Dn (full subject DN, e.g. "CN=alice,O=DMZ-API")
    """
...
def _require_api_project(request: Request, project: str) -> str:
    """
    Resolve and authorise the caller for the given project.

    Flow:
      1. Validate the project code (3 uppercase alnum).
      2. Read the client cert CN from proxy headers.
      3. Look up the project bound to that CN via key_manager.
      4. Reject if no binding exists or if it doesn't match the path.

    Returns the CN on success. Raises HTTPException(403) on any failure
    so callers get the same opaque rejection for every mismatch.
    """
    ...
    if key_manager is None:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Key manager not configured"
        )

    bound_project = key_manager.get_project_for_cn(cn)
...
@router.get(
    "/payloads/{project}",
    summary="List payload templates for a project",
    response_description="Names of .json files directly under the project dir",
)
async def list_templates(project: str, request: Request) -> dict[str, Any]:
    """List template names (without .json) for the authorised project."""
    _require_api_project(request, project)
    return {"project": project.upper(), "templates": payload_store.list_payloads(project.upper())}


@router.get(
    "/payloads/{project}/schemas",
    summary="List JSON Schemas for a project",
    response_description="Names of .json files under {project}/Schemas/",
)
async def list_project_schemas(project: str, request: Request) -> dict[str, Any]:
    """List schema names (without .json) for the authorised project."""
    _require_api_project(request, project)
...
async def send_template(
    ...
    """
    Load a template, merge optional overrides, regenerate identifiers,
    validate, and forward through the gateway.

    Request body (all optional):
      {
        "overrides":  {"Data": {"k": "v"}, ...}   shallow-merge into template
        "verbatim":   true                         skip ID/Date regeneration
      }
    """
...
async def send_schema(project: str, request: Request, payload: dict[str, Any]):
    """
    Caller supplies the full payload. Every schema under
    {project}/Schemas/ must pass before the message is forwarded.
    """
    request_id = request.state.request_id
    project = project.upper()
    _require_api_project(request, project)
    ...
    # Force the path project onto the payload — prevents cross-project send.
