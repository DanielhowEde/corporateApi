"""
LOW-SIDE DMZ API Service.

This service handles message exchange with the DMZ Gateway.
...
async def lifespan(app: FastAPI):
    ...
    # Shutdown
    logger.info("Shutting down LOW-SIDE DMZ API")
    await gateway_client.close()


app = FastAPI(
    title="LOW-SIDE DMZ API",
    description="Low-side API for secure message exchange via DMZ Gateway",
    version="1.0.0",
    lifespan=lifespan
)
...
async def send_message(request: Request, message: Dict[str, Any]) -> SuccessResponse:
    ...
    This endpoint validates the message schema and forwards it to the Gateway.
...
async def receive_message(request: Request, message: Dict[str, Any]) -> SuccessResponse:
    ...
    This endpoint validates the message schema and writes it to disk atomically.
